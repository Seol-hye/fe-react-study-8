    ...
    reducers : { // 각종 함수들 (getter, setter)
        saveUserId(state, action){      // saveUserId('abcd')
            ...
            return action.payload // 'abcd'         setUserId('abcd')
                        // 단순 텍스트를 리턴 ---> state 변수값에 저장할 값으로 전달 ---> state에 저장
        },
        clearUserId(state){     // clearUserId()
            return "clear"; // "clear" 텍스트값 return -> state 에 저장
        },
        saveUserInfo(state, action){ //action.payload   객체(json) 단위로 값 전달받았다
...
//날씨 정보 저장 상태관리
// let [weather, setWeather] = useState( {weather:"", temperature:15, hmdt:40 } );
...
let weatherSlice = createSlice({
    ...
    reducers : {
        ...
        setWeatherInfo(state, action){  //state 현재 원본 상태값(객체라서 주소값), action 전달 온 매개변수
...
/*
    ...
    Dispatch : Action을 담은 reducer 함수를 -> Store 쪽에 전달하는 함수
