...
function Login() {
    ...
    /*
        ...
        페이지 이동 등 다른 페이지에서도 로그인 여부 체크? --> redux 등록된 사용자 정보
    ...
    */
    ...
    let weather = useSelector((state)=> { return state.weather }); // store에 등록된 weather 상태값
    console.log(weather);
        ...
        <div>
            ...
            <div>
                <h3>redux 값 테스트
                    <div>
                        <button onClick={()=>{
                            //saveUserId('abcd')); 단순함수호출X
                            dispatch(saveUserId('abcd')); // redux 관련 action 생성 호출
                        }}>SaveUserId 호출</button>

                        <button onClick={()=>{
                            dispatch(clearUserId());
                        }}>ClearUserId 호출</button>
            ...
            <div>
                <button onClick={()=>{
                    ...
                    if(token != null && token != ''){ //단순텍스트로 응답
                        //발급된 엑세스토큰 전달 받음
                        // 저장 -> 다음에 요청할 때 토큰값을 같이 담아서 요청 (자유이용권 제시 입장)
